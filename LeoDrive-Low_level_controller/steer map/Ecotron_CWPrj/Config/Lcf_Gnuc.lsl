/* Default linker script, for normal executables  20211020_3->20211022_1*/
...
LCF_DSPR2_START = 0x50018000;
LCF_DSPR2_SIZE = 24k;

LCF_DSPR1_START = 0x60000000;
LCF_DSPR1_SIZE = 120k;

LCF_DSPR0_START = 0x70000000 + 0x8000 + 0;
LCF_DSPR0_SIZE = 112k - 0x8000 - 0;
...
/*
LCF_INTVEC0_START = 0x803F0000;
LCF_TRAPVEC0_START = 0x80010100;
LCF_TRAPVEC1_START = 0x803F6200;
LCF_TRAPVEC2_START = 0x803F6000;
*/
...
__CAL_ON_RAM_START = 0x70000000;
__CAL_ON_SIZE  = 0x8000;

__NVM_RAM_START = 0x50014000;
__NVM_INIT_START = 0x80230000;
__NVM_LEN = 0x4000;

__FNVM_DATA_END_MIN = 0x50014000;
__FNVM_DATA_END_MAX = 0x50015FFC;

__UFNVM_DATA_END_MIN = 0x50016000;
__UFNVM_DATA_END_MAX = 0x50017FFC;

__UDS_PROGRAME_ADDR = 0x70105FFE;
__UDS_PROGRAME_LEN  = 0x02;

RESET = 0x80010020;

MEMORY
{
	cal_ram_local (w!xp): org = 0xd0000000, len = 0x8000 + 0
	cal_ram (w!xp): org = 0x70000000, len = 0x8000 + 0
	
	adc_ram_local (w!xp): org = 0xd0000000, len = 0x200
	adc_ram (w!xp): org = 0x50000000, len = 0x200
	
    msr_ram_local (w!xp): org = 0xd0000200, len = 80K - 0x200
	msr_ram (w!xp): org = 0x50000200, len = 80K - 0x200
	
	write_back_fixed_ram_local (w!xp): org = 0xd0014000, len = 8K
	write_back_fixed_ram (w!xp): org = 0x50014000, len = 8K
	
	write_back_unfixed_ram_local (w!xp): org = 0xd0016000, len = 8K
	write_back_unfixed_ram (w!xp): org = 0x50016000, len = 8K	
	
	dsram2_local (w!xp): org = 0xd0018000, len = 24K
	dsram2 (w!xp): org = 0x50018000, len = 24K
...
*/	
  	...
  	dsram0_local (w!xp): org = 0xd0000000 + 0x8000 + 0, len = 112K - 0x8000 - 0
  	dsram0 (w!xp): org = 0x70000000 + 0x8000 + 0, len = 112K - 0x8000 - 0

  	psram0 (w!xp): org = 0x70100000, len = 23K
	psram_local (w!xp): org = 0xc0000000, len = 23K
  	
  	main2bootParam (w!xp): org = 0x70105C00, len = 1K
    main2bootParam_local (w!xp): org = 0xc0005C00, len = 1K  	
  	
  	/*====pfls0======*/
  	
	pfls0 (rx!p): org = 0x80010000, len = 2M - 64K       /*BootLoader 64K */
	pfls0_nc (rx!p): org = 0xa0010000, len = 2M - 64K
	
	/*====pfls1======*/
	
	/*cal_rom_old (rx!p): org = 0x80200000, len = 0x10000*/        /*max 64K*/
	/*cal_rom_nc_old (rx!p): org = 0xa0200000, len = 0x10000*/

	cal_rom (rx!p): org = 0x80200000, len = 0x8000 + 0        /*max 64K*/
	cal_rom_nc (rx!p): org = 0xa0200000, len = 0x8000 + 0
	
	boot_cal_rom (rx!p): org = 0x80210000, len = 16K        
	boot_cal_rom_nc (rx!p): org = 0xa0210000, len = 16K     
	
	write_back_fixed_rom (rx!p): org = 0x80214000, len = 8K      /*used 8K, Reserved 8K(org=0x80216000) */     
	write_back_fixed_rom_nc (rx!p): org = 0xa0214000, len = 8K  
	
	write_back_unfixed_rom (rx!p): org = 0x80218000, len = 8K     /*used 8K, Reserved 8K(org=0x8021A000) */  
	write_back_unfixed_rom_nc (rx!p): org = 0xa0218000, len = 8K  	
	
	fnvm_flag_rom (rx!p): org = 0x8021C000, len = 8K        /*used 8K, Reserved 8K(org=0x8021E000) */  
	fnvm_flag_rom_nc (rx!p): org = 0xa021C000, len = 8K
	
	ufnvm_flag_rom (rx!p): org = 0x80220000, len = 2K        /*used 2K(used 0x80220100 for First PowerOn Flag ), Reserved 30K */ 
	ufnvm_flag_rom_nc (rx!p): org = 0xa0220000, len = 2K
	
	poweron_flag_rom (rx!p): org = 0x80228000, len = 1K        /*used 1K, Reserved 31K */  
	poweron_flag_rom_nc (rx!p): org = 0xa0228000, len = 1K
	
	
	fnvm_init (rx!p): org = 0x80230000, len = 8K      /*used 8K, Reserved 8K(org=0x80216000) */     
	fnvm_init_nc (rx!p): org = 0xa0230000, len = 8K  
	
	unfnvm_init (rx!p): org = 0x80232000, len = 8K     /*used 8K, Reserved 8K(org=0x8021A000) */  
	unfnvm_init_nc (rx!p): org = 0xa0232000, len = 8K 

   	epk_data (rx!p): org = 0x80234000, len = 0x100 
	epk_data_nc (rx!p): org = 0xa0234000, len = 0x100

	pfls1 (rx!p): org = 0x80234100, len = 2M-192K-16K-0x100   
	pfls1_nc (rx!p): org = 0xa0234100, len = 2M-192K-16K-0x100
	/*====dfls0======*/
...
__MSR_RAM_START = ORIGIN(msr_ram);
__MSR_RAM_LEN =LENGTH(msr_ram);	

/* map local memory address to a global address */ 
REGION_MAP( CPU0 , ORIGIN(dsram0_local), LENGTH(dsram0_local), ORIGIN(dsram0))
REGION_MAP( CPU0 , ORIGIN(cal_ram_local), LENGTH(cal_ram_local), ORIGIN(cal_ram))
...
REGION_MAP( CPU2 , ORIGIN(adc_ram_local), LENGTH(adc_ram_local), ORIGIN(adc_ram))
REGION_MAP( CPU2 , ORIGIN(msr_ram_local), LENGTH(msr_ram_local), ORIGIN(msr_ram))  
REGION_MAP( CPU2 , ORIGIN(write_back_fixed_ram_local), LENGTH(write_back_fixed_ram_local), ORIGIN(write_back_fixed_ram))
REGION_MAP( CPU2 , ORIGIN(write_back_unfixed_ram_local), LENGTH(write_back_unfixed_ram_local), ORIGIN(write_back_unfixed_ram))
...
SECTIONS
{
	...
	.__calibration_rom (0x80200000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__calibration_rom));
	} > cal_rom

	.__bootcal_rom (0x80210000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__bootcal_rom));
	} > boot_cal_rom  
	
	.__writeback_fixed_rom (0x80214000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__writeback_fixed_rom));
		
	} > write_back_fixed_rom
	
	.__writeback_unfixed_rom (0x80218000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__writeback_unfixed_rom));
	
	} > write_back_unfixed_rom 
	
	
	.__fnvm_flag (0x8021C000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__fnvm_flag));
		. = ALIGN(0x2000);
	} > fnvm_flag_rom = 0xFF 		
	
	.__ufnvm_flag (0x80220000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__ufnvm_flag));
		. = ALIGN(0x100);
	} > ufnvm_flag_rom = 0xFF 	
	...
	.__poweron_flag (0x80228000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__poweron_flag));
		. = ALIGN(0x100);
	} > poweron_flag_rom = 0xFF 
	
	
	/*==ram==*/

	.cal_data_on (0x70000000): FLAGS(arsl)/*FLAGS(awz)*/
	{
		KEEP (*(.cal_data_on));
	. = ALIGN(0x8000);
	} > cal_ram AT > cal_rom = 0xFF
	__CAL_SEG1_SIZE_GET = SIZEOF(.cal_data_on);	
	
	__CAL_SEG1_START = 0x70000000;
	__CAL_SEG1_SIZE = __CAL_SEG1_SIZE_GET;
	.__vadc_ram (0x50000000): FLAGS(awz)
	{
		KEEP (*(.__vadc_ram));
	} > adc_ram AT > pfls1
	
	.measure_data (0x50000200): FLAGS(awz)
	{
		KEEP (*(.measure_data));
	} > msr_ram AT > pfls1
	
	.__fnvm_data (0x50014000): FLAGS(arsl)
	{
        KEEP (*(.__fnvm_data));
		__FNVM_DATA_END = .;
        . = ALIGN(0x2000);
	} > write_back_fixed_ram AT > fnvm_init = 0x00 
	
	.__writeback_ram (0x50016000): FLAGS(arsl)
	{
        KEEP (*(.__writeback_ram));
		__UFNVM_DATA_END = .;
		. = ALIGN(0x2000);
	} > write_back_unfixed_ram  AT > unfnvm_init = 0x00	
	
    .__epk_data (0x80234000) : FLAGS(arsl)/*FLAGS(arl)*/
	{
		KEEP (*(.__epk_data));
		. = ALIGN(0x100);
	} > epk_data = 0x20
